...
def geometric_median(points):
    ...
    """
    # Initialize the estimate of the geometric median
    ...
    while True:
        # Compute the distances between each point and the estimate of the geometric median
        distances = np.linalg.norm(points - estimate, axis=1)

        # Compute the inverse of the distances, avoiding division by zero
        inv_distances = np.where(distances > 0, 1 / distances, 0)

        # Compute the weighted sum of the points
        weighted_sum = np.sum(points * inv_distances[:, np.newaxis], axis=0)

        # Update the estimate of the geometric median
        new_estimate = weighted_sum / np.sum(inv_distances)

        # Check for convergence
        if np.linalg.norm(new_estimate - estimate) < threshold:
            break
...
for t in range(T):
    ...
    for i in range(N):
        if i not in malicious_nodes:
            predecessors = list(G_copy.predecessors(i))
            num_predecessors = len(predecessors)
            if num_predecessors > 0:
                weights = [w_local[j] for j in predecessors] + [w_local[i]]
                biases = [b_local[j] for j in predecessors] + [b_local[i]]
                median_weights = geometric_median(weights)
                median_biases = geometric_median(biases)
                w_local[i] = median_weights
                b_local[i] = median_biases
            else:
                w_local[i] = w_local[i]
                b_local[i] = b_local[i]

    # calculate the average error of w and b for only benign nodes
    benign_nodes = [node for node in G.nodes if node not in malicious_nodes]
...
plt.title("Geometric median, malicious = 90")
