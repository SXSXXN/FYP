    ...
    for i in range(N):
        for j in G.predecessors(i):
            if i not in malicious_nodes:  # benign agent (follows the strategy)
                ...
                # beta[i, j] = weighted_alpha_sum / sum([alpha_decay ** (
                #    len(previous_alpha[j])-k-1) for k in range(len(previous_alpha[j]))]) - 0.5
...
plt.title("Without Forgetting Factor")
